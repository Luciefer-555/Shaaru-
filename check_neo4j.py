...
def main():
    ...
    for lbl in labels:
        res = kg.query(f"MATCH (n:{lbl}) RETURN count(n) AS c")
        print(f"  {lbl:<12}: {res[0]['c']}")

    res_cap = kg.query("MATCH (p:Product) WHERE p.caption IS NOT NULL AND p.caption <> '' RETURN count(p) AS c")
    ...
    for r in rels:
        res = kg.query(f"MATCH ()-[rel:{r}]->() RETURN count(rel) AS c")
